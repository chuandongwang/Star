...
using BAH.PI.BD.Core.Const.Enum;
using Kingdee.BOS;
...
namespace PHMX.K3.BD.App.ServicePlugIn.Stock
{
    [Description("从ERP同步仓库数据至WMS")]
    //同步字段：名称+编码
    public class SynchronizeStockInformation : AbstractOperationServicePlugIn
    {
        ...
        public override void AfterExecuteOperationTransaction(AfterExecuteOperationTransaction e)
        {
            ...
            string targetFormId = PIBDFormPrimaryKey.Instance.Warehouse();
            ...
            foreach (var data in unmatchDataEntities)
            {
                ...
                targetBillService.UpdateValue("FNumber", -1, string.Concat(data.FieldProperty<DynamicObject>(this.BusinessInfo.GetField("FUseOrgId")).BDNumber(), data.BDNumber()));
                targetBillService.UpdateValue("FName", -1, new LocaleValue(data.BDName(this.Context)));
                targetBillService.UpdateValue("FShape", -1, WarehouseShape.Instance.Logic());
