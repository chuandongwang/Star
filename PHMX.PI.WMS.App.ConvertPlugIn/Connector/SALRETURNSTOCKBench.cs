...
{
    [Description("销售退货单，单据工作台插件")]
    public class SALRETURNSTOCKBench : AbstractBillBenchPlugIn
    {
        public override void AfterCreateTargetData(AfterCreateTargetDataEventArgs e)
        {
            if (!e.Rule.TargetFormId.EqualsIgnoreCase("SAL_RETURNSTOCK")) return;
            ...
            foreach (var entry in entryArray)
            {
                ...
                for (int i = 0; i < sources.Count(); i++)
                {
                    ...
                    if (materialField.Adaptive(field => this.View.Model.GetValue(field, rowIndex).AsType<DynamicObject>().FieldRefProperty<DynamicObject>(field, "FAuxUnitId")) != null &&
                        materialField.Adaptive(field => this.View.Model.GetValue(field, rowIndex).AsType<DynamicObject>().FieldRefProperty<DynamicObject>(field, "FAuxUnitId")).PkId<int>() != this.View.Model.GetValue("FAuxUnitId", rowIndex).AsType<DynamicObject>().PkId<int>())
                    {
                        billService.SetItemValueByID("FAuxUnitId", materialField.Adaptive(field => this.View.Model.GetValue(field, rowIndex).AsType<DynamicObject>().FieldRefProperty<DynamicObject>(field, "FAuxUnitId")), rowIndex);
                    }//end if

                    //库存辅单位数量
                    if (materialField.Adaptive(field => this.View.Model.GetValue(field, rowIndex).AsType<DynamicObject>().FieldRefProperty<DynamicObject>(field, "FAuxUnitId")) != null)
                    {
                        billService.UpdateValue("FAuxUnitQty", rowIndex, item.Qty);
                    }//end if
