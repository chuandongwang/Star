...
{
    [Description("采购退料单，单据工作台插件")]
    public class PURMRBBench : AbstractBillBenchPlugIn
    {
        public override void AfterCreateTargetData(AfterCreateTargetDataEventArgs e)
        {
            if (!e.Rule.TargetFormId.EqualsIgnoreCase("PUR_MRB")) return;
            ...
            foreach (var entry in entryArray)
            {
                ...
                for (int i = 0; i < sources.Count(); i++)
                {
                    ...
                    //实退数量。
                    billService.UpdateValue("FRMREALQTY", rowIndex, item.Cty > 0 ? item.Cty : item.Qty);
