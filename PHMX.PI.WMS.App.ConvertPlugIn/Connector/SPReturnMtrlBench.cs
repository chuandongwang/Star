...
{
    [Description("简单生产退料单,单据工作台插件")]
    public class SPReturnMtrlBench : AbstractBillBenchPlugIn
    {
        public override void AfterCreateTargetData(AfterCreateTargetDataEventArgs e)
        {
            if (!e.Rule.TargetFormId.EqualsIgnoreCase("SP_ReturnMtrl")) return;
            ...
            foreach (var entry in entryArray)
            {
                ...
                for (int i = 0; i < sources.Count(); i++)
                {
                    ...
                    {
                        billService.UpdateValue("FSecStockQty", rowIndex, item.Qty);
                    }//end if
                    ...
                    {
                        billService.SetItemValueByID("FLot", item.LotNo, rowIndex);
                    }//end if
                    ...
                    //重量
                    billService.UpdateValue("FPHMXWgt", rowIndex, item.PHMXWgt);
