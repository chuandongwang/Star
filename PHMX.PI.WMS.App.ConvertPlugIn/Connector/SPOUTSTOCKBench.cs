...
{
    [Description("简单生产退库单,单据工作台插件")]
    public class SPOUTSTOCKBench : AbstractBillBenchPlugIn
    {
        public override void AfterCreateTargetData(AfterCreateTargetDataEventArgs e)
        {
            if (!e.Rule.TargetFormId.EqualsIgnoreCase("SP_OUTSTOCK")) return;
            ...
            foreach (var entry in entryArray)
            {
                ...
                for (int i = 0; i < sources.Count(); i++)
                {
                    ...
                    //实发数量。
                    billService.UpdateValue("FOutQty", rowIndex, item.Cty > 0 ? item.Cty : item.Qty);
                    ...
                    if (materialField.Adaptive(field => this.View.Model.GetValue(field, rowIndex).AsType<DynamicObject>().FieldRefProperty<DynamicObject>(field, "FAuxUnitId")) != null)
                    {
                        billService.UpdateValue("FSecOutQty", rowIndex, item.Qty);
                    }//end if
