...
{
    [Description("简单生产入库单，单据工作台插件。")]
    public class SPInStockBench : AbstractBillBenchPlugIn
    {
        public override void AfterCreateTargetData(AfterCreateTargetDataEventArgs e)
        {
            if (!e.Rule.TargetFormId.EqualsIgnoreCase("SP_InStock")) return;
            ...
            foreach (var entry in entryArray)
            {
                ...
                for (int i = 0; i < sources.Count(); i++)
                {
                    ...
                    {
                        billService.UpdateValue("FSecRealQty", rowIndex, item.Qty);
                    }//end if
