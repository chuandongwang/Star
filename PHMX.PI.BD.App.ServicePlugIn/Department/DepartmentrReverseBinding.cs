...
namespace PHMX.PI.BD.App.ServicePlugIn.Department
{
    [Description("普华智造部门，与ERP部门绑定关系，操作插件。")]
    public class DepartmentReverseBinding : AbstractOperationServicePlugIn
    {
        public override void AfterExecuteOperationTransaction(AfterExecuteOperationTransaction e)
        {
            ...
            var deptMetadata = FormMetaDataCache.GetCachedFormMetaData(this.Context, mirrorField.LookUpObject.FormId);
            var deptBusinessInfo = deptMetadata.BusinessInfo.GetSubBusinessInfo("FPHMXContactId");
            var deptField = deptBusinessInfo.GetField("FPHMXContactId").AsType<BaseDataField>();
            ...
            var dataEntities = e.SelectedRows.Select(data => data.DataEntity)
                                ...
                                .Select(data =>
                                {
                                    var dept = data.FieldProperty<DynamicObject>(mirrorField);
                                    deptField.RefIDDynamicProperty.SetValue(dept, data.PkId<string>());//先把对应的主键赋值过去。
                                    return dept;
                                }).ToArray();
            ...
            para.FilterClauseWihtKey = "FMasterId in (Select FID From TABLE(fn_StrSplit(@MasterIds,',',1))) and FDEPTID <> FMasterId";
            ...
            dataEntities.Join(allocateEntities,
                              ...
                              (left, right) =>
                              {
                                  deptField.RefIDDynamicProperty.SetValue(right, left.FieldRefIdProperty<string>(deptField));
                                  return right;
                              }).ToArray();
