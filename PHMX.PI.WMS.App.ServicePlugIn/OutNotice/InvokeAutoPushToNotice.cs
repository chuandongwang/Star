...
namespace PHMX.PI.WMS.App.ServicePlugIn.OutNotice
{
    [Description("发货通知，源单审核后(事务外)自动下推，操作插件。")]
    public class InvokeAutoPushToNotice: AbstractOperationServicePlugIn
    {
        ...
        public override void AfterExecuteOperationTransaction(AfterExecuteOperationTransaction e)
        {
            ...
            var metas = this.FormOperation.AppBusinessService.Where(item => item.IsEnabled)
                                                             ...
                                                             .Where(item => item.TargetFormId.EqualsIgnoreCase(PIWMSFormPrimaryKey.Instance.OutNotice()))
                                                             .ToArray();
            if (!metas.Any()) return;
