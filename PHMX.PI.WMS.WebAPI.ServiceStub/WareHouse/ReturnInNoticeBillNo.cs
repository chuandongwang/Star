...
{
    public class ReturnInNoticeBillNo : AbstractWebApiBusinessService
    {
        public ReturnInNoticeBillNo(KDServiceContext context) : base(context)
        {
        }
        /// <summary>
        /// 获取未完成收货通知和部分收货的收货通知
 ...
 T8.FID AS FCONTACTID,T8.FNUMBER AS FCONTACTNUMBER,T9.FNAME AS FCONTACTNAME,T8.fformid AS FCONTACTFORMID,sum(t2.FHASINBOUNDMQTY)AS FHASINBOUNDMQTYSUM,'未收货' AS FStatus, COUNT(t.FENTRYID) as COUNT
                ...
                AND T0.FWHID LIKE '%{0}%' AND T0.FOWNERID LIKE '%{2}%' AND T30.FAREAID LIKE '%{1}%' AND T2.FHASINBOUNDMQTY = 0 
                ...
                SELECT t0.FBILLNO,t0.FCREATEDATE AS FCREATEDATE,t4.FID as FWHID,T4.FNUMBER AS FWHNUMBER,T5.FNAME AS FWHNAME,
 T6.FID AS FOWNERID,T6.FNUMBER AS FOWNERNUMBER,T7.FNAME AS FOWNERNAME ,
 T8.FID AS FCONTACTID,T8.FNUMBER AS FCONTACTNUMBER,T9.FNAME AS FCONTACTNAME,T8.fformid AS FCONTACTFORMID,sum(t2.FHASINBOUNDMQTY)AS FHASINBOUNDMQTYSUM,'部分收货' AS FStatus, COUNT(t.FENTRYID) as COUNT
                FROM  dbo.BAH_T_WMS_INNOTICE t0
                LEFT JOIN dbo.BAH_T_WMS_INNOTICEENTRY t ON t0.FID = t.FID
	            LEFT JOIN dbo.BAH_T_BD_LOCATION_L t1 ON t.FLOCID = t1.FID
	            LEFT JOIN dbo.BAH_T_WMS_INNOTICEENTRY_W t2 ON t.FENTRYID = t2.FENTRYID
 ...
 T8.FID AS FCONTACTID,T8.FNUMBER AS FCONTACTNUMBER,T9.FNAME AS FCONTACTNAME,T8.fformid AS FCONTACTFORMID,sum(t2.FHASINBOUNDMQTY)AS FHASINBOUNDMQTYSUM,'未收货' AS FStatus, COUNT(t.FENTRYID) as COUNT
	            ...
	            INNER JOIN dbo.BAH_V_BD_CONTACT_L T9 ON T8.FID = T9.FID
                WHERE t.FMANUALCLOSE = 'A' AND t2.FJOINSTATUS = 'A' AND t0.FDOCUMENTSTATUS = 'C'
                AND T0.FWHID LIKE '%{0}%' AND T0.FOWNERID LIKE '%{1}%'  AND T2.FHASINBOUNDMQTY = 0 
                AND t0.FBILLNO not in
                (
                SELECT t0.FBILLNO
                FROM  dbo.BAH_T_WMS_INNOTICE t0
