...
{
    public class CreateOutNoticeAndPickDetailByMaterial : AbstractWebApiBusinessService
    {
        public CreateOutNoticeAndPickDetailByMaterial(KDServiceContext context) : base(context)
        {
        }
        /// <summary>
        /// 扫描
        ...
        public ServiceResult ExecuteService(string data)
        {
            ...
            try
            {
                ...
                foreach (var items in FinalInput)
                {
                    ...
                    List<PickDetailLinkInDetailDto.PickDetailEntryLinkInNotice> pushdatas = new List<PickDetailLinkInDetailDto.PickDetailEntryLinkInNotice>();
                    
                    for (int i = 0; i < items.FinalInputDataGroupByContactId.Count(); i++)
                    {
                        PickDetailLinkInDetailDto.PickDetailEntryLinkInNotice pushdata = new PickDetailLinkInDetailDto.PickDetailEntryLinkInNotice();
                        pushdata.SourceEntryId = (long)billView.Model.GetEntryPKValue("FEntity", i);
                        pushdata.SourceBillId = (long)billView.Model.GetPKValue();            
                        pushdata.ToLocId = items.FinalInputDataGroupByContactId[i].FToLocId;
                        pushdata.PHMXWgt = items.FinalInputDataGroupByContactId[i].FPHMXWgt;
                        pushdata.ToTrackNo = items.FinalInputDataGroupByContactId[i].FToTrackNo;
