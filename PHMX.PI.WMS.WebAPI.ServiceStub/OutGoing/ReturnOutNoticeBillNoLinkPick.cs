...
{
    public class ReturnOutNoticeBillNoLinkPick : AbstractWebApiBusinessService
    {
        public ReturnOutNoticeBillNoLinkPick(KDServiceContext context) : base(context)
        {
        }
        /// <summary>
        /// 获取部分拣货和全部拣货的发货通知(要加跟踪号过滤条件)
        /// </summary>
        ...
        public ServiceResult ExecuteService(string whid,string areaid,string ownerid, string trackno)
        {
            ...
            try
            {
                ...
                string sqlSelect = string.Format(@"/*dialect*/
                                                      SELECT DISTINCT CASE t0.FDIRECTION WHEN 'General' THEN '普通' ELSE '退回' END AS FDIRECTION, 
 ...
 T8.FID AS FCONTACTID,T8.FNUMBER AS FCONTACTNUMBER,T9.FNAME AS FCONTACTNAME,T8.fformid AS FCONTACTFORMID,'全部拣货' AS FStatus, COUNT(t.FENTRYID) as COUNT
                ...
                LEFT JOIN dbo.BAH_T_WMS_PICKUPENTRY_W T10 ON T0.FBILLNO = T10.FSOURCEBILLNO
	            LEFT JOIN dbo.BAH_T_WMS_PICKUPENTRY_B T11 ON T10.FENTRYID = T11.FENTRYID
                WHERE  t0.FDOCUMENTSTATUS = 'C' AND T11.FTOTRACKNO  like '%{3}%'
 ...
 T8.FID AS FCONTACTID,T8.FNUMBER AS FCONTACTNUMBER,T9.FNAME AS FCONTACTNAME,T8.fformid AS FCONTACTFORMID,'全部拣货' AS FStatus, COUNT(t.FENTRYID) as COUNT
                ...
                LEFT JOIN dbo.BAH_T_WMS_PICKUPENTRY_W T10 ON T0.FBILLNO = T10.FSOURCEBILLNO
	            LEFT JOIN dbo.BAH_T_WMS_PICKUPENTRY_B T11 ON T10.FENTRYID = T11.FENTRYID
                WHERE  t0.FDOCUMENTSTATUS = 'C' AND T11.FTOTRACKNO  like '%{2}%'
