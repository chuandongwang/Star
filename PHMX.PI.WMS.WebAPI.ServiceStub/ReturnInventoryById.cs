...
{
    public class ReturnInventoryById : AbstractWebApiBusinessService
    {
        public ReturnInventoryById(KDServiceContext context) : base(context)
        {
        }
        /// <summary>
        /// 获取库存明细
        /// </summary>
        ///参数为三个：物料ID ，货主ID，仓库ID
        ...
        public ServiceResult ExecuteService(string materialid, string ownerid,string whid)
        {
            ...
            try
            {
                ...
                string sqlSelect = string.Format(@"/*dialect*/
                 SELECT DISTINCT t.FID,t.FBILLNO AS FTrackNo,FLOCID,t6.FNUMBER AS FLocNumber,t5.FNAME AS FLocName, t.FPACKAGEID,t4.FNUMBER AS FPackageNumber ,t7.FNAME AS FPackageName,FMQTY,t8.FNAME AS FMainUnitId,FLOTNO,FPRODUCEDATE,FEXPIRYDATE
                ,CASE T10.FIGNOREINVENTORYTRACKNO WHEN 1 then 'True' ELSE 'False' END AS FIGNOREINVENTORYTRACKNO,t3.FSPECIFICATION, t.FEXPPERIOD,'D' AS FEXPUNIT
                ,T.FAVGCTY,T.FCTY
                ...
                INNER JOIN dbo.BAH_T_BD_MATWAREHOUSE T11 ON T11.FID = T.FMATERIALID
	            where t.FMATERIALID = '{0}' and t.FOWNERID = '{1}' and t.FWHID = '{2}' and  (t9.FUSE = 'Storage' or t9.FUSE = 'Pick')
