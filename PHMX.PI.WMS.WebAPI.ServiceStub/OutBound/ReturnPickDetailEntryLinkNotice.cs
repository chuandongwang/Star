...
{
    public class ReturnPickDetailEntryLinkNotice : AbstractWebApiBusinessService
    {
        public ReturnPickDetailEntryLinkNotice(KDServiceContext context) : base(context)
        {
        }
        /// <summary>
        /// 输入发货通知编号物料编码，输出相关联的拣货通知信息
        /// </summary>
        /// <param name="billno">拣货通知编号</param>
        /// <returns>返回服务结果。</returns>
        public ServiceResult ExecuteService(string entryid)
        {
            ...
            try
            {
                ...
                string sqlSelect2 = string.Format(@"/*dialect*/
                                                    SELECT distinct t.FENTRYID fid, t0.FBILLNO fbillno 
                                                    ...
                                                    AND ((T2.FJoinMQty < T.FTOMQTY AND T11.FENABLECAPACITY = 0) OR (T11.FENABLECAPACITY = 1 AND (T2.FJOINCTY < T.FTOCTY ))) 
                                                    AND  t2.FSOURCEID = '{0}'
                                                    AND T30.FUSE ='Transition'
                                                                                                           
                 ;",entryid);// or a.num is null
                //TODO:通过平台动态引擎获取数据
               ...
               SELECT T11.FBILLNO,t.FID,t.FENTRYID, T0.FSEQ, t.FTOLOCID, t6.FNUMBER AS FlocIdNumber, t1.FNAME AS FlocIdName
	            ...
	            AND t.FENTRYID = '{0}'
                AND ((T5.FJoinMQty < T.FTOMQTY AND T8.FENABLECAPACITY = 0) OR (T8.FENABLECAPACITY = 1 AND (T5.FJOINCTY < T.FTOCTY)))
                AND T30.FUSE ='Transition'
